...
export default function MyInquiries() {
  ...
  if (user.role === 'AGENT') {
    ...
    const handleSendResponse = (inquiryId: number) => {
      const response = responseText[inquiryId];
      if (!response || response.trim() === '') return;
      
      // TODO: Implement actual API call to send response
      console.log(`Sending response to inquiry ${inquiryId}:`, response);
      alert('Response sent successfully!');
      setResponseText({ ...responseText, [inquiryId]: '' });
      setSelectedInquiry(null);
    };

    const handleMarkAsClosed = (inquiryId: number) => {
      // TODO: Implement actual API call to mark as closed
      console.log(`Marking inquiry ${inquiryId} as closed`);
      alert('Inquiry marked as closed');
    };

    // Agent view with client information
    const agentInquiries = inquiries.map(inq => ({
      ...inq,
      clientName: 'John Doe', // TODO: Get from actual data
      clientEmail: 'client@example.com',
      clientPhone: '+94 77 123 4567'
    }));

    const filteredAgentInquiries = agentInquiries.filter(inq => {
      if (filter === 'all') return true;
      return inq.status === filter;
    });
        ...
        <main className="max-w-7xl mx-auto px-6 py-8 space-y-6">
          ...
          <div className="space-y-3">
            {filteredAgentInquiries.length === 0 ? (
              ...
              filteredAgentInquiries.map((inquiry) => (
                <div key={inquiry.id} className="bg-white shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow duration-300">
                  <div className="p-5">
                    <div className="flex gap-4">
                      {/* Property Image */}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="bg-blue-50 p-2.5 mb-2 border-l-2 border-blue-500">
                          ...
                          <div className="text-xs text-gray-700 space-y-0.5">
                            <div><span className="font-medium">Name:</span> {inquiry.clientName}</div>
                            <div><span className="font-medium">Email:</span> {inquiry.clientEmail}</div>
                            <div><span className="font-medium">Phone:</span> {inquiry.clientPhone}</div>
                          </div>
                        </div>
                          ...
                          <div className="mt-3 space-y-3">
                            ...
                            {inquiry.agentResponse ? (
                              ...
                              <div className="space-y-2">
                                ...
                                <textarea
                                  value={responseText[inquiry.id] || ''}
                                  onChange={(e) => setResponseText({ ...responseText, [inquiry.id]: e.target.value })}
                            ...
                            <div className="flex gap-2 pt-2">
                              <button 
                                onClick={() => window.open(`/properties/${inquiry.id}`, '_blank')}
